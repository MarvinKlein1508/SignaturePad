...
export default class Sigpad {
    ...
    _setListeners() {
        var instance = this;
        this._element.addEventListener("mousedown", function (mouseEvent) { instance._onStart(instance, mouseEvent); });
        ...
        this._element.addEventListener("touchstart", function (mouseEvent) { instance._onStart(instance, mouseEvent); });
    ...
    _onStart(s, e) {
        if (!s._imgReady || !s._bgReady) return;
        ...
        if (s._element.classList.contains('disabled')) {
            return;
        }

        var mousePos = s._getRelativeMousePos(e);
        if (mousePos != null) {
            s._currentStroke.push(mousePos);
            s._drawing = true;

            const event = new CustomEvent('sigpad.start', { detail: s.getImage() });
            s._element.dispatchEvent(event);
        }
    }

    _onEnd(s, e) {
        if (!s._drawing) return;
        ...
        const event = new CustomEvent('sigpad.finish', { detail: s.getImage() });
        s._element.dispatchEvent(event);
    }
    ...
    _onMove(s, e) {
        ...
        if (s._drawing) {
            var pos = s._getRelativeMousePos(e);
            if (pos != null) {
                s._currentStroke.push(pos);
    ...
    render(force) {
        if (this._drawing || this._newBgImage || this._newImage || force || this._element.width != this._element.clientWidth || this._element.height != this._element.clientHeight) {
            this._element.width = this._element.clientWidth;
            this._element.height = this._element.clientHeight;
